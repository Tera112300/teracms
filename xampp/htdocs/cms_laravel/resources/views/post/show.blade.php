...
<section class="content">
  <div class="card card-default show_card">
    ...
    <div class="card-body">
      ...
      {{-- <div class="callout callout-info">
        <h5>コンテンツ</h5>
        <p>{{$posts_id->post_content}}</p>
      </div> --}}
      <div class="callout callout-info">
        <h5>抜粋文</h5>
        <p>{{$posts_id->post_excerpt}}</p>
      </div>
      <div class="callout callout-info">
        <h5>表示状態</h5>
        <p>{{$posts_id->post_status}}</p>
      </div>
      <div class="callout callout-info">
        <h5>作成日時</h5>
        <p>{{$posts_id->created_at}}</p>
      </div>
      <div class="callout callout-info">
        <h5>更新日時</h5>
        <p>{{$posts_id->updated_at}}</p>
      </div>
      ...
      <div class="callout callout-info">
        ...
        {{-- <p>{{$posts_id->post_name}}</p> --}}
      </div>
      <div class="callout callout-info">
        <h5>パーマリンク</h5>
        <p class="permalink"><a href="{{url('?p='.$posts_id->id)}}" rel="external noreferrer noopener" target="_blank">{{url('?p='.$posts_id->id)}}</a></p>
      </div>
      <div class="callout callout-info">
        <h5>投稿ステータス</h5>
        <p>{{$posts_id->post_status}}</p>
      ...
      <div class="callout callout-info">
        ...
        <p>
          ...
          <img src="{{asset('upload/'.$posts_id->post_guid)}}" alt="" width="360">
          @else
          未設定です。
          @endif
        </p>
      ...
      <div class="callout callout-info">
        <h5>seo タイトル</h5>
        <p>{{$posts_id->seo_title}}</p>
      </div>
      <div class="callout callout-info">
        <h5>meta 説明</h5>
        <p>{{$posts_id->meta_description}}</p>
      </div>
      <div class="callout callout-info">
        <h5>meta キーワード</h5>
        <p>{{$posts_id->meta_keywords}}</p>
      </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content bg-danger">
      ...
      <div class="modal-footer justify-content-between">
        ...
        <form action="{{url('/cms-admin/post/delete/'.$posts_id->id)}}" method="POST">
          {{ csrf_field() }}
            {{ method_field('DELETE') }}
          <button class="btn btn-danger">削除</button>
        </form>
        {{-- <a href="{{url('/cms-admin/post/delete/'.$posts_id->id)}}" class="btn btn-danger">削除</a> --}}
